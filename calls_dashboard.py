...
if uploaded_file is not None:
    ...
    with tab2:
        ...
        if start_date == end_date:
            st.info(f"📅 **Filtro Attivo:** Stai visualizzando i dati analitici del giorno: **{data_inizio_it}**")
        else:
            st.info(f"📅 **Filtro Attivo:** Stai visualizzando il periodo dal **{data_inizio_it}** al **{data_fine_it}**")
    ...
    with tab3:
        ...
        if tot_ghosts > 0:
            ...
            df_ghosts['Data/Ora Formattata'] = pd.to_datetime(df_ghosts['datetime']).dt.strftime('%d/%m/%Y %H:%M:%S')
            
            cols_to_show = ['Data/Ora Formattata', 'customer_number', 'waiting_seconds', 'missed_call_reason']
            cols_available = [c for c in cols_to_show if c in df_ghosts.columns]
            
            st.dataframe(
                df_ghosts[cols_available].rename(
                    columns={'Data/Ora Formattata': 'Data e Ora', 'customer_number': 'Num. Cliente', 'waiting_seconds': 'Attesa (s)', 'missed_call_reason': 'Ragione'}
                ), 
                use_container_width=True
            )
        else:
